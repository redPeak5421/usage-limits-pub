...
ChatGPT/Codex 共用 `.openai` 卡：三种主题的折叠卡均隐藏可用重置，展开后在额度条下方显示次数与到期明细；平铺缩略卡共享自然高度，不因供应商补充信息而单独增高。
分享由独立的「重置次数」开关控制可用次数及最早到期的最多三条明细，与「明细」开关互不影响；按到期正序排列，以设备时区 `yyyy-MM-dd HH:mm:ss` 显示。预览与导出共用布局，静态图片不使用滚动列表。新设置默认关闭，旧设置按原「明细」选择迁移，此后独立保存。仅选中 ChatGPT / Grok 的内置账号时显示此开关；独立于首页展开状态，零次照常显示但不带日期，未知次数不编造。不展示已用历史。
当前卡片仅显示可用次数与到期日期：展开按 `expires_at` 正序显示三行日期，超出部分内部滚动；不再展示已用列表、历史时间范围、重复标题或每条“1 次”。零次只显示总数，不显示过期日期。历史解析与缓存兼容保留。可用项按 credit id 在内存去重；快照仅保存排序后的日期数组 `availableExpirations` / `usedDates`，不保存原始 ID。缺失或无效的到期日期不编造列表行，总次数仍以接口字段为准。旧缓存缺少数组时保留摘要展示。
