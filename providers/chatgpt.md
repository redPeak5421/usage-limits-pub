...
展开明细参照即梦：可用列表按 `expires_at` 正序，已用列表按 `occurred_at` 倒序；各最多露出三行，超出部分内部滚动，不截断已获取记录。可用项同样按 credit id 在内存去重；快照仅保存排序后的日期数组 `availableExpirations` / `usedDates`，不保存原始 ID。缺失或无效的到期日期不编造列表行，总次数仍以接口字段为准。旧缓存缺少数组时保留摘要展示。
